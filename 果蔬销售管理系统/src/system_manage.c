...
static Coord settingPagePos = { 10,15 }, settingPageRectSize = { 4,60 }, systemManageMenuPos = { 18,15 };
...
void timeSetting(FVMO *gdata) {
	...
	while (1) {
		...
		drawMenu(gdata->renderer, timeSettingMenuPos, (Coord) { 5, 30 },"时间设定", 4, 1,
			"向后一天",
			"向后三十天", 
			"直接设定时间",
			"退出");
...
void systemManage(FVMO *gdata) {
	...
	while (1) {
		...
		drawMenu(gdata->renderer, systemManageMenuPos, (Coord) { 5, settingPageRectSize.y },"系统管理", 4, 1,
