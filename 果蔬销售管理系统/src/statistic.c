...
static const Coord InvCheckPos = { 2,3 }, financeCheckPos = { 22,4 }, statsCheckMenuPos = { 26,3 };
...
void statsCheckPage(FVMO *gdata) {
	...
	while (1) {
		...
		drawMenu(gdata->renderer, statsCheckMenuPos, (Coord) {5,InvCheckRectSize.y}, "数据核对", 4, 1,
			"上一页",
			"下一页",
			"查看记录详情",
			"退出");
...
void statsPage(FVMO *gdata) {
	...
	while (1) {
		...
		drawMenu(gdata->renderer, statsMenuPos, (Coord) {4,statsRectSize.y}, "数据统计", 7, 1,
			"上一页",
			"下一页",
			"查看商品记录",
